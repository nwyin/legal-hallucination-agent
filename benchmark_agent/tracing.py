...
def redact(value):
    ...
    if isinstance(value, dict):
        return {k: "[REDACTED]" if any(p in str(k).upper() for p in
                ("API_KEY", "SECRET", "PASSWORD", "AUTHORIZATION", "ACCESS_TOKEN")) else redact(v)
...
class CheckedExporter(OTLPSpanExporter):
    ...
    def export(self, spans):
        ...
        spans = [ReadableSpan(name=redact(s.name), context=s.context, parent=s.parent, resource=s.resource,
                    attributes=s.attributes, kind=s.kind, start_time=s.start_time, end_time=s.end_time,
                    instrumentation_scope=s.instrumentation_scope, links=s.links,
                    events=[Event(e.name, redact(dict(e.attributes or {})), e.timestamp) for e in s.events],
                    status=Status(s.status.status_code, redact(s.status.description))) for s in spans]
        try:
...
if enabled and base_url:
    ...
    exporter = CheckedExporter(endpoint=f"{base_url.rstrip('/')}/api/public/otel/v1/traces",
                               headers={"Authorization": f"Basic {auth}"}, timeout=30)
langfuse = Langfuse(tracing_enabled=enabled, base_url=base_url, sample_rate=1.0,
                    mask=lambda data: redact(data), mask_otel_spans=mask_otel_spans, span_exporter=exporter,
...
def require_capture():
    ...
    if (not enabled or os.getenv("LANGFUSE_CAPTURE_CONTENT", "true").lower() != "true"
        ...
        raise TelemetryError("Experiments require full, unsampled Langfuse capture; remove telemetry disable/redaction settings")
    try:
...
def export_lifecycle():
    ...
    try:
        ...
        try:
            ...
            raise BaseExceptionGroup("Execution and telemetry export failed", [application_error, export_error]) from None
        raise
    else:
        flush_traces()
...
def provenance(config):
    ...
    files += sorted((root / "scripts").rglob("*.py")) + [root / "pyproject.toml", root / "uv.lock"]
    return redact({"effective_config": config, "provider": "openrouter", "code_revision": git("rev-parse", "HEAD"),
...
def experiment(config):
    ...
    try:
        with export_lifecycle(), propagate_attributes(session_id=metadata["run_id"],
        ...
        def invoke():
            try:
                ...
                if episode_trace.get() and not capture_failures.failures:
                    ...
                    score_metrics({**metrics, "error_count": 1}, episode_trace.get(), bound.arguments["example_id"])
                raise
        try:
...
def score_metrics(metrics, trace_id, example_id=None):
    for name, value in metrics.items():
        if isinstance(value, (int, float)):
            langfuse.create_score(name=name, value=float(value), data_type="NUMERIC", trace_id=trace_id,
...
def publish_aggregate(results):
    ...
    metrics = aggregate_metrics([evaluate_hallucination_entry(
        {"list_hallucinations": r.get("true_answer") or [],
         "predicted_hallucinations": r.get("predicted_hallucinations")}) for r in results])
    ...
    with langfuse.start_as_current_observation(name="experiment-summary", input=[r["example_id"] for r in results],
                                               metadata=run_context.get(), output=metrics) as span:
...
def capture_http(name, method, url, **request):
    ...
    with langfuse.start_as_current_observation(name=name, as_type="retriever",
            input=redact({"method": method, "url": url, **request})) as span:
        def capture(response):
            span.update(output=redact({"status_code": response.status_code, "body": response.text,
