...
def setup_experiment_logging(dataset: str, method: str, model_id: str, example_id: str, experiments_dir: str = "outputs/experiments"):
    ...
    log_filepath = os.path.join(log_dir, f"{example_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
def episode_opinion_cache_dir(paths_config: Dict[str, Any], output_dir: str, model_id: str,
                              method: str, max_steps: int, example_id: str) -> str:
    """Per-episode opinion cache, so parallel runs never clear each other's opinions."""
    base = paths_config.get('opinion_cache_dir') or os.path.join(output_dir, 'opinion_cache')
...
def create_agent(
    ...
    if method == 'boed':
        ...
        params['belief_update_prompt_constructor'] = BOEDCitationTrackerBeliefUpdatePromptConstructor(TASK_DOMAIN_KNOWLEDGE)
        ...
        while not environment.is_terminated() and not environment.is_truncated():
            ...
            for attempt in range(3):
                ...
                if action is not None:
                    break
                logger.warning("Action parsing failed at step %s (attempt %s/3). Retrying select_action.", step_num, attempt + 1)
            if action is None:
                logger.error("Action parsing failed after 3 attempts at step %s. Terminating episode.", step_num)
                break
            ...
            if action_type == "PROVIDE_FINAL_RESPONSE":
                ...
                if not final_response and final_response_retries < 2:
                    ...
                    logger.warning("PROVIDE_FINAL_RESPONSE returned empty or unparseable list (attempt %s/2). Prompting agent to retry.",
...
def run_single_example(
    ...
    environment = create_environment({
        ...
        'opinion_cache_dir': episode_opinion_cache_dir(paths_config, output_dir, model_id, method, max_steps, example_id),
    })
    agent = create_agent(method, environment, model_api or ModelAPI(), model_config, agent_config)

    metrics_collector = MetricsCollector(save_dir=episode_metrics_dir(metrics_dir, dataset, model_id, method, max_steps))
    metrics_collector.start_episode(
        episode_id=example_id, task_name=TASK_NAME, agent_type=agent.__class__.__name__,
        environment_info={}, method=method, model_id=model_id,
    )
...
def main(cfg: DictConfig):
    ...
    for example in tqdm(examples, desc=f"Running {TASK_NAME}/{method}"):
        ...
        try:
            results.append(run_single_example(
                method=method, example=example, paths_config=paths_config, search_config=search_config,
                env_settings=env_settings, model_config=model_config, agent_config=agent_config,
