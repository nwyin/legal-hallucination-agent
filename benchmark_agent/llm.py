...
class ModelAPI:
    ...
    def __call__(self, model_id: str, prompt: list[dict[str, str]], **kwargs):
        langfuse.update_current_span(input={"messages": prompt, "model": model_id, **kwargs},
                                     metadata={"provider": "openrouter"})
        try:
            response = get_client().chat.completions.create(
                messages=prompt, model=model_id, **kwargs
            )
            langfuse.update_current_span(output=response.model_dump(mode="json") if hasattr(response, "model_dump")
