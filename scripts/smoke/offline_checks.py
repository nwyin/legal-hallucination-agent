...
def make_environment(cache_dir):
    return HallucinationCheckerEnvironment(
        brief_info={
            "filename": "offline",
            "list_hallucinations": ["Fake v. Case, 1 U.S. 1 (2099)"],
        },
...
def action_registry_is_complete():
    ...
    assert (
        description
        == get_action_class(ActionType.OPEN_COURTLISTENER_SEARCH).description
    )
    options = get_action_class(ActionType.OPEN_COURTLISTENER_SEARCH).inputs[
        "search_type"
    ]["description"]
...
def environment_exposes_every_action():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert set(env.action_space) == set(ActionType), set(ActionType) ^ set(
            env.action_space
        )
        assert env.initial_observation.metadata["available_actions"] == [
...
def opinion_access_registers_document():
    ...
    environment.fetch_opinion = lambda opinion_id: {
        "id": opinion_id,
        "plain_text": text,
        "case_name": "Fake v. Case",
    }
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            read = env.step(
                actions.ReadDocument(
                    opinion_id="opinion_4242", start_line=0, num_lines=3
                )
            )
            assert "line 0" in str(read.result) and "line 3" not in str(read.result)

            found = env.step(
                actions.SearchLocalOpinion(opinion_id="4242", search_string="line 7")
            )
...
def scratchpad_dispatch():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        obs = env.step(
            actions.EditScratchpad(operation="append", content="second note")
        )
        assert obs.result["scratchpad"] == "first note\nsecond note", obs.result
        env.step(
            actions.EditScratchpad(operation="insert", content="zeroth", position=0)
        )
        obs = env.step(
            actions.EditScratchpad(operation="replace", content="2nd", position=2)
        )
        assert obs.result["scratchpad"] == "zeroth\nfirst note\n2nd", obs.result
        obs = env.step(
            actions.EditScratchpad(operation="replace", content="x", position=9)
        )
...
def date_parsing_and_cutoff_filtering():
    ...
    dated = web_search.SearchResult(
        title="a", url="u", snippet="s", source="serpapi", published_date="2020-01-01"
    )
    undated = web_search.SearchResult(title="b", url="u", snippet="s", source="serpapi")
    kept = web_search._filter_by_cutoff_date(
        [dated, undated], date(2021, 1, 1), exclude_undated=True
    )
    assert [r.title for r in kept] == ["a"], [r.title for r in kept]
    dropped = web_search._filter_by_cutoff_date(
        [dated], date(2019, 1, 1), exclude_undated=True
    )
    assert dropped == []
...
def search_client_makes_http_requests():
    ...
    get.assert_called_once_with(
        "https://example.invalid/search", params={"q": "test query"}
    )
...
def web_search_keeps_undated_results():
    ...
    web_search.SerpApiClient.google_search = lambda self, query, **kw: {
        "organic_results": [
            {"title": "undated", "link": "u", "snippet": "s", "position": 1}
        ]
    }
    try:
        with (
            tempfile.TemporaryDirectory() as tmp,
            patch.dict(os.environ, {"SERPAPI_API_KEY": "offline"}),
        ):
            env = make_environment(tmp)
            obs = env.step(actions.OpenWebSearch(query="fake v case"))
    finally:
...
def action_parsing_round_trip():
    ...
    assert parameters == {"query": "fake v case"}, (
        parameters
    ...
    for bad in (
        '{"action": {"action_type": "OPEN_WEB_SEARCH", "query": "   "}}',
        '{"action": {"action_type": "NOT_AN_ACTION"}}',
        "no json here",
    ):
        try:
...
def action_selection_reasks_on_invalid_output():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        agent = BOEDCitationTrackerAgent(
            environment=make_environment(tmp),
            model_api=fake_model_api,
            model_id="offline",
            open_web_search_enabled=True,
        )
        action = agent._call_llm_for_action_selection(
    ...
    assert (
        calls[1][1]["role"] == "assistant"
        and "not a valid action" in calls[1][2]["content"]
    )
...
def action_selection_rejects_disabled_actions():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        agent = BOEDCitationTrackerAgent(
            ...
            model_api=fake_model_api,
            model_id="offline",
            open_web_search_enabled=False,
        )
        action = agent._call_llm_for_action_selection(
            [{"role": "user", "content": "choose"}]
        )
    assert isinstance(action, actions.Think), action
    assert len(calls) == 2 and "not available in this run" in calls[1][2]["content"], (
        calls[1]
    )
