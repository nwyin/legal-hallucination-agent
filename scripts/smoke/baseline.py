...
from benchmark_agent.evaluation import compute_metrics, evaluate_entry
...
def serialize(value):
    ...
    if hasattr(value, "action_type"):
        return {
            ...
            "parameters": value.get_input_parameters(),
        }
    if hasattr(value, "result") and hasattr(value, "metadata"):
        return {
            "result": serialize(value.result),
            "metadata": serialize(value.metadata),
        }
    return value
...
def main():
    ...
    if recording and (base / "manifest.json").exists():
        raise SystemExit(
            "Baseline already exists; choose a new --directory to avoid overwriting it."
        )
    ...
    logging.basicConfig(
        level=logging.INFO, filename=out / "execution.log", filemode="w"
    )
    config = {
    ...
    if recording:
        ...
        sources = list((ROOT / "benchmark_agent").rglob("*.py")) + list(
    ...
    for budget in config["step_budgets"]:
        ...
        def completion(*, tape=tape, tape_path=tape_path, budget=budget, **request):
            nonlocal index
            ...
            if recording:
                ...
                if (
                    index >= len(tape)
                    or tape[index]["requested"] != request
                    or tape[index]["effective"] != effective
                ):
                    raise AssertionError(
                        f"Request mismatch at steps={budget}, call={index}"
                    )
        ...
        llm_module.get_client = lambda: SimpleNamespace(
        ...
        if not recording:
            ...
            expected = json.loads(
                (base / "record" / f"steps_{budget}_summary.json").read_text()
            )
            assert summary == expected, (
                f"Summary/trajectory mismatch for {budget} steps"
            )
        results.append(
            {
                "max_steps": budget,
                "calls": index,
                "actions": [s["action"]["action_type"] for s in summary["history"]],
                "prediction": summary["predicted_hallucinations"],
                "f1": summary["f1"],
            }
        )
    scoring = {
        "empty": compute_metrics(*evaluate_entry([], [])),
        "substring": compute_metrics(
            *evaluate_entry(["invented citation"], ["invented citation, extra context"])
        ),
    }
