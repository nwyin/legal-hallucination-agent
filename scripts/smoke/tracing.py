...
import json
import os
from contextlib import chdir, nullcontext, suppress
...
def main():
    ...
    parser.add_argument(
        "--export",
        action="store_true",
        help="Export synthetic traces to the configured project",
    )
    ...
    def collect(exporter, spans):
        ...
        return (
            original_export(exporter, spans)
            if args.export
    ...
    with (
        ...
        chdir(cache_dir),
        patch.object(OTLPSpanExporter, "export", collect),
        block_network() if not args.export else nullcontext(),
    ):
        import httpx

        from benchmark_agent import llm, run, tracing
        from benchmark_agent.actions import ActionType
        from benchmark_agent.tracing import enabled, flush_traces, langfuse, observe

        scores = []
        original_score = langfuse.create_score
        from langfuse._task_manager.score_ingestion_consumer import (
        ...
        if not args.export:
            patch.object(
                ScoreIngestionConsumer,
                "_upload_batch",
                lambda self, batch: score_events.extend(batch),
            ).start()
        ...
        def respond(request):
            ...
            assert json.loads(request.content) == recorded["requested"], (
                "Model-facing request changed"
            )
        ...
        with (
            ...
            try:
                summary = run.run_single_example(
                    ...
                    env_settings={"max_steps": 3},
                    model_config=config["model"],
                    agent_config=config["agent"],
                    output_dir=output,
                    metrics_dir=output,
                    example_id="synthetic-langfuse-smoke",
                    dataset="tracing-smoke",
                )
            finally:
                flush_traces()
                client.close()
        ...
        assert any(
            s["name"] == "f1" and s["trace_id"] == summary["langfuse_trace_id"]
            for s in scores
        )
        assert any(
            s["name"] == "example_count"
            and s["trace_id"] != summary["langfuse_trace_id"]
            for s in scores
        )
        assert all(
            s["metadata"]["run_id"] == summary["langfuse_run_id"] for s in scores
        )
        ...
        assert all(s.parent is None or s.parent.span_id in span_ids for s in spans), (
        ...
        action_spans = {
            step["action"].action_type.value.lower().replace("_", "-")
            for step in summary["history"]
        }
        expected_spans = {"select-action", "update-beliefs", *action_spans}
        if (
            summary["total_steps"] >= 3
        ):  # the prediction prompt only runs when the step budget forces it
        ...
        if not args.export:
            ...
            try:
                with langfuse.start_as_current_observation(
                    name="check-masking", input=os.environ["TRACING_TEST_API_KEY"]
                ):
                    pass
                flush_traces()
                ...
                flush_traces()
                error_span = next(
                    s for s in exported if s.name == "check-exception-masking"
                )
                assert "secret-for-masking-check" not in str(
                    error_span.status.description
                )
                assert "secret-for-masking-check" not in str(
                    [dict(e.attributes) for e in error_span.events]
                )
        ...
        opinion = {
            "id": 123,
            "plain_text": "Opinion text. " * 500 + "RAW_OPINION_TAIL",
            "html": "<p>Raw HTML</p>",
        }
        search_payload = {
            "count": 2,
            "results": [{"id": 1, "snippet": "visible result"}],
            "unfiltered": "RAW_SEARCH_FIELD",
        }
        web_payload = {
            "organic_results": [
                {
        ...
        with (
            ...
            langfuse.start_as_current_observation(name="synthetic-retrieval") as root,
        ):
            env = HallucinationCheckerEnvironment(
                brief_text="synthetic",
                brief_info={},
                max_steps=5,
                opinion_cache_dir=cache,
            )
            with (
                patch.object(
                    courtlistener.requests,
                    "get",
                    return_value=http_response(opinion),
                ),
                patch.object(courtlistener.time, "sleep"),
            ):
            ...
            with (
                patch.object(
                    courtlistener.requests,
                    "get",
                    return_value=http_response(search_payload),
                ),
                patch.object(courtlistener.time, "sleep"),
            ):
                env.step(OpenCourtListenerSearch(query="synthetic"))
            with (
                patch.dict(os.environ, {"SERPAPI_API_KEY": "synthetic-serp-secret"}),
                patch.object(
                    web_search.requests,
                    "get",
                    return_value=http_response(web_payload),
                ),
            ):
        ...
        raw_spans = [
            s
            for s in exported
            if s.name in {"request-courtlistener", "request-serpapi"}
        ]
        content = str([dict(s.attributes) for s in raw_spans])
        assert all(
            marker in content
            for marker in ("RAW_OPINION_TAIL", "RAW_SEARCH_FIELD", "RAW_WEB_FIELD")
        )
        ...
        if not args.export:
            ...
            with (
                ...
                try:
                    run.run_single_example(
                        method=config["method"],
                        example=example,
                        paths_config={},
                        search_config=config["search"],
                        env_settings={"max_steps": 0},
                        model_config=config["model"],
                        agent_config=config["agent"],
                        output_dir=output,
                        metrics_dir=output,
                        example_id="failed-synthetic",
                        dataset="tracing-smoke",
                    )
                ...
                assert any(
                    s["trace_id"] == failed_id and s["name"] == "error_count"
                    for s in scores
                )
                assert any(
                    format(s.context.trace_id, "032x") == failed_id for s in exported
                )
                (Path(output) / "unexported.json").write_text(
                    '{"langfuse_exported": false}'
                )
                (Path(output) / "exported.json").write_text(
                    '{"langfuse_exported": true}'
                )
            ...
            with patch.object(langfuse, "flush", wraps=langfuse.flush) as flush:
                try:
                    with (
                        tracing.export_lifecycle(),
                        langfuse.start_as_current_observation(name="failed-episode"),
                    ):
            ...
            with patch.object(
                OTLPSpanExporter, "export", return_value=SpanExportResult.FAILURE
            ):
                try:
                    with (
                        tracing.export_lifecycle(),
                        langfuse.start_as_current_observation(name="failed-export"),
                    ):
                        pass
            ...
            with patch.object(
                langfuse,
                "flush",
                side_effect=RuntimeError("synthetic flush failure"),
            ):
                try:
            ...
            assert any(
                e["body"]["traceId"] == trace_id and e["body"]["name"] == "f1"
                for e in score_events
            )
            with patch.object(
                ScoreIngestionConsumer,
                "_upload_batch",
                side_effect=RuntimeError("synthetic score export failure"),
            ):
