    ...
    with (
        ...
        try:
            ...
            def environment(settings):
                ...
                env.action_space = [
                    action for action in env.action_space if action in allowed
                ]
            ...
            spans = [
                s for s in exported if format(s.context.trace_id, "032x") == trace_id
            ]
            ...
            assert all(
                s.parent is None or s.parent.span_id in span_ids for s in spans
            ...
            if not args.export:
                ...
                try:
                    ...
                    assert "secret-for-masking-check" not in str(
                        dict(masked.attributes)
                    )
                    assert (
                        "[REDACTED]" in masked.attributes["langfuse.observation.input"]
                    )
                    try:
                        with langfuse.start_as_current_observation(
            ...
            with (
                ...
                with (
                    patch.dict(
                        os.environ, {"SERPAPI_API_KEY": "synthetic-serp-secret"}
                    ),
                    patch.object(
                        web_search.requests,
                        "get",
            ...
            if not args.export:
                ...
                with (
                    ...
                    assert any(
                        format(s.context.trace_id, "032x") == failed_id
                        for s in exported
                    )
