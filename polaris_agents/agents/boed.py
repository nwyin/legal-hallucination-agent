"""
BOED Agent implementing Bayesian Optimal Experimental Design with single-layer beliefs.

Task-agnostic agent that:
- Maintains beliefs over θ (task parameters) only
- Selects actions to maximize expected information gain about θ
- Uses injected prompt constructors for domain-specific prompting

This is a simpler variant of IDS-OED that does not maintain design beliefs (D).
"""
    ...
    def get_current_prediction(self) -> Tuple[Optional[str], Optional[float]]:
        ...
        try:
            ...
            try:
                ...
                is_local_provider = self.provider in ('local', 'della')
